...
// diagonal distance
float diag(int i0, int j0, int i1, int j1) {
  return std::max(std::abs(i0 - i1), std::abs(j0 - j1));
}

// manhattan distance
float man(int i0, int j0, int i1, int j1) {
  return std::abs(i0 - i1) + std::abs(j0 - j1);
}
// BS to get shit to python
    ...
    for (int i = 0; i < 8; ++i) {
      if (nbrs[i] >= 0) {
        ...
        if (new_cost < costs[nbrs[i]]) {
          ...
          if (diag_ok) {
            heuristic_cost = diag(nbrs[i] / w, nbrs[i] % w,
                                       goal    / w, goal    % w);
          }
          else {
            heuristic_cost = man(nbrs[i] / w, nbrs[i] % w,
                                     goal    / w, goal    % w);
          }
